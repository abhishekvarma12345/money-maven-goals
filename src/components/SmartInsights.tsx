...
const SmartInsights = () => {
  ...
  return <Card className="card-hover rounded-lg">
        ...
        <Tabs defaultValue={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="spending" className="space-y-4">
            <div className="p-4 bg-muted rounded-lg">
              ...
              {topCategories.length > 0 ? <ul className="space-y-2">
                  {topCategories.map((category, index) => <li key={index} className="flex justify-between items-center p-2 rounded-md bg-background">
                      <span className="capitalize">{category.category}</span>
                      <span className="font-medium">{category.percentage}%</span>
                    </li>)}
                </ul> : <p className="text-sm text-muted-foreground">No spending data available yet</p>}
            </div>
            <div className="text-sm text-muted-foreground">
              {topCategories.length > 0 && <p>Your highest spending is on <span className="font-medium capitalize">{topCategories[0].category}</span>, 
          ...
          <TabsContent value="trends" className="space-y-4">
            <div className="p-4 bg-muted rounded-lg">
              ...
              <div className="flex items-center gap-2">
                {trend === 'decreasing' && <div className="flex items-center text-budget-success">
                    <TrendingDown className="mr-1 h-4 w-4" />
                    <span className="font-medium">Decreasing</span>
                  </div>}
                {trend === 'increasing' && <div className="flex items-center text-budget-danger">
                    <TrendingUp className="mr-1 h-4 w-4" />
                    <span className="font-medium">Increasing</span>
                  </div>}
                {trend === 'stable' && <div className="flex items-center text-budget-info">
                    <span className="font-medium">Stable</span>
                  </div>}
              </div>
            </div>
            <div className="text-sm text-muted-foreground">
              {trend === 'decreasing' && <p>Great job! Your spending is trending downward compared to last month.</p>}
              {trend === 'increasing' && <p>Your spending is trending higher than last month. Consider reviewing your budget.</p>}
              {trend === 'stable' && <p>Your spending pattern is relatively stable compared to previous months.</p>}
            </div>
          </TabsContent>
          ...
          <TabsContent value="savings" className="space-y-4">
            <div className="p-4 bg-muted rounded-lg">
              ...
              {savingsOpp.category !== 'none' ? <div className="p-2 rounded-md bg-background">
                  ...
                  <div className="w-full bg-gray-200 dark:bg-gray-700 h-1.5 rounded-full mt-1">
                    <div className="bg-budget-success h-1.5 rounded-full" style={{
                  width: '20%'
                }}></div>
                  </div>
                </div> : <p className="text-sm text-muted-foreground">Add more expenses to get personalized savings tips</p>}
            </div>
